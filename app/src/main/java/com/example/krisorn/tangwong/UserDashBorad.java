...
import com.example.krisorn.tangwong.AdminDashBorad.UserDashBoradAdapter;
...
public class UserDashBorad extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ActionBarDrawerToggle toggle = new ActionBarDrawerToggle(
                UserDashBorad.this, drawer, toolbar, R.string.navigation_drawer_open, R.string.navigation_drawer_close);
        drawer.addDrawerListener(toggle);
        toggle.syncState();
        NavigationView navigationView = (NavigationView) findViewById(R.id.nav_view_dash_board);
        Log.d("testSideNav","---------------");
        navigationView.setNavigationItemSelectedListener(UserDashBorad.this);
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    mDatabase.child("room").child(roomLiveNow).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            ...
                            countOrderNow= 0;
                            for(int i2 =0;i2<dataSnapshot.child("feature").getChildrenCount();i2++){
                                String feature = dataSnapshot.child("feature").child(String.valueOf(i2)).getValue(String.class);
                                if(dataSnapshot.child(feature).child("typeOfFetureShow").getValue(String.class).equals("user")
                                || dataSnapshot.child(feature).child("typeOfFetureShow").getValue(String.class).equals("both")){
                                    countOrderNow++;
                                }

                            }
                            Log.d("listUserDashBoard", String.valueOf(countOrderNow));
                            adapter = new UserDashBoradAdapter(countOrderNow,UserDashBorad.this);
